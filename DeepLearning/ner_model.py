...
from util import ConfusionMatrix, Progbar, minibatches,test_ner
from data_util import get_chunks
from model import Model
from defs import LBLS

logger = logging.getLogger("rnn_ner_model")
...
class NERModel(Model):
    ...
    def fit(self, sess, saver, train_examples_raw, dev_set_raw,dev_raw):
        best_score = 0.
        ...
        for epoch in range(self.config.n_epochs):
            ...
            score = self.run_epoch(sess, train_examples, dev_set, train_examples_raw, dev_set_raw,dev_raw)
            if score > best_score:
                best_score = score
                logger.info("new best f1 score:{:>.3f}".format(score))
                if saver:
                    logger.info("Saving model in %s", self.config.model_output)
