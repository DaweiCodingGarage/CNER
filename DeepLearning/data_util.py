...
from defs import LBLS, NONE, LMAP, UNK, EMBED_SIZE
...
P_CASE = "CASE:"
CASES = ["ag", "a", "ad", "an","b","c","dg","d","df","f","g","nrfg", \
"i","j","k","l","m","mq", "ng","n","nr","ns","nt","nz","nrt","h","O", "o","p","q","r", \
"s","tg","t","u","uj","ul","uv","uz","ug","ud", "vg","v","vd","vn","vq","eng","zg" ,"w","x","y","yg", "z","un"]
START_TOKEN = "<s>"
END_TOKEN = "</s>"
HEAD = [1,0]
...
class ModelHelper(object):
    ...
    def vectorize_example(self, sentence, labels=None):
        sentence_ = [self.tok2id.get(word[0], self.tok2id[UNK]) for word in sentence]
    ...
    def build(cls, data):
        ...
        tok2id = build_dict((word for sentence, _ in data for word in sentence), offset=1, max_words=10000)
        #tok2id.update(build_dict(["postag:"+c for c in CASES], offset=len(tok2id)))
        #tok2id.update(build_dict([c for c in HEAD], offset=len(tok2id)))
...
def load_embeddings(args, helper):
    ...
    for word, vec in load_word_vector_mapping(args.vocab, args.vectors).items():
        ...
        if word in helper.tok2id:
            #print(helper.tok2id[word],len(embeddings))
